...
def jqrs(ecg, HRVparams):
    """
    QRS detection based on the Pan-Tompkins method adapted for Python. This offline implementation is designed for analyzing 
    ECG signals to identify QRS complexes, which are indicative of heartbeats.

    Parameters:
    ecg : array_like
        One ECG channel signal to run the detector on, provided in millivolts (mV).
    HRVparams : dict
        A dictionary containing the configuration parameters for heart rate variability (HRV) analysis, including:
        - 'PeakDetect': Dictionary of peak detection parameters like REF_PERIOD, THRES.
        - 'Fs': Sampling frequency of the ECG data.

    Returns:
    qrs_pos : array_like
        Indices of the detected QRS peaks within the ECG signal (in samples).
    sign : float
        The sign of the detected peaks, indicating whether they are predominantly positive or negative within the ECG signal.
    en_thres : float
        The energy threshold used for detecting QRS complexes, based on the signal's characteristics.
    
    The algorithm preprocesses the ECG signal with a band-pass filter to attenuate noise outside the QRS frequency band. Then, it applies 
    a derivative filter to highlight the slopes of the QRS complexes. The squared magnitudes of these filtered signals are integrated over 
    a moving window to accentuate regions of high activity corresponding to QRS complexes. Adaptive thresholding is then used to identify potential 
    QRS regions, which are further refined to pinpoint the exact location of QRS peaks.
    """
    ...
    # Bandpass filtering and preprocessing
    ...
    if np.sum(np.abs(bpfecg) > MIN_AMP) / NB_SAMP > 0.20:
        ...
        if SIGN_FORCE:
            ...
            loc = np.zeros(nb_s)
            for j in range(nb_s):
                segment = np.abs(bpfecg[left[j]:right[j]])
                if len(segment) != 0:
